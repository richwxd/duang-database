...
public class MongoIndexUtils {
    ...
    public static void createIndex(DBCollection coll, Class<?> cls) throws Exception {
        ...
        for (int i = 0; i < fields.length; i++) {
            ...
            boolean isVoField = isVoField(fields[i]);
            boolean isVoCollField = isVoCollField(fields[i]);
...
//        fields[i].isAnnotationPresent(VoColl.class);
        Class<?> type = field.getType();
        return DataType.isSetType(type) || DataType.isListType(type) || DataType.isMapType(type) || DataType.isQueueType(type);
    }
    ...
    private static void createVoIndexKey(Class<?> cls, String key) throws Exception {
        ...
        if (ToolsKit.isNotEmpty(fields)) {
            for (Field field : fields) {
                boolean isVoField = isVoField(field); //field.isAnnotationPresent(Vo.class);
                boolean isVoCollField = isVoCollField(field); //field.isAnnotationPresent(VoColl.class);
                if (isVoField || isVoCollField) {
                    Class<?> clazz = null;
                    if (isVoField) {
                        clazz = field.getType();
